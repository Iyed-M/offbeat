...
func runSpotifySync(configPath, homeDir string) int {
	...
	fmt.Fprintf(os.Stdout, "Spotify desired state committed (revision %d): %d playlists, %d playlist entries, %d Liked Songs entries, %d supported entries, %d unsupported entries.\n", result.StateRevision, result.PlaylistCount, result.PlaylistEntryCount, result.LikedSongsEntryCount, result.SupportedEntryOccurrences, result.UnsupportedEntryOccurrences)
	return 0
}
...
func decodeSpotifySyncResult(result any) (ipc.SpotifySyncResult, error) {
	...
	if !decoded.Changed || decoded.StateRevision != 1 || decoded.PlaylistCount < 0 || decoded.PlaylistEntryCount < 0 || decoded.LikedSongsEntryCount < 0 || decoded.SupportedEntryOccurrences < 0 || decoded.UnsupportedEntryOccurrences < 0 {
